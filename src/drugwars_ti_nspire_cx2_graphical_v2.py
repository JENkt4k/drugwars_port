...
# Embedded image resources. Luna packs the BMPs into the .tns.
# TI-Nspire image naming can vary by document/resource handling, so each
# asset loader tries the BMP filename, the basename, and insertion order.
ASSETS = {}
ASSET_SPECS = [
    ("header_skyline", "header_skyline.bmp", "1"),
    ("police_car",     "police_car.bmp",     "2"),
    ("mugger",         "mugger.bmp",         "3"),
    ("dead_body",      "dead_body.bmp",      "4"),
    ("trenchcoat",     "trenchcoat.bmp",     "5"),
    ("doctor",         "doctor.bmp",         "6"),
    ("gun",            "gun.bmp",            "7"),
    ("gun_snub",       "gun_snub.bmp",       "8"),
    ("gun_44",         "gun_44.bmp",         "9"),
    ("subwaybg",       "subwaybg.bmp",       "10"),
]

def load_assets():
    global ASSETS
    ASSETS = {}
    for key, filename, insertion in ASSET_SPECS:
        image = None
        # Luna preserves the resource filenames in the TNS. TI Python may
        # expose them by filename, basename, or insertion-order number.
        for candidate in (filename, key, insertion):
            try:
...
def message_with_asset(title_text, lines, asset_name, accent=YELLOW,
                       asset_x=178, asset_y=48, footer_text="ENTER: Continue"):
    begin_screen()
    header(title_text)
    shown = show_asset(asset_name, asset_x, asset_y)
    # Keep text on the left when artwork is available; otherwise use full width.
    y = 48
    for i, s in enumerate(lines):
        c = accent if i == 0 else WHITE
        text(10, y, s, c)
        y += 22
    if not shown:
        text(10, 158, "[art unavailable]", GRAY)
    footer(footer_text)
    end_screen()
    return wait_enter_or_esc()
...
class Game:
    ...
    def title_screen(self):
        while True:
            ...
            # Use the embedded skyline banner when available. Fall back to
            # the original procedural skyline so the game still runs if an
            # image resource is missing or named differently.
            if not show_asset("header_skyline", 0, 0):
                rect(0, 0, W, H, BLACK)
            ...
            # Dark panel beneath the 318x60 skyline asset.
            rect(0, 60, W, 130, BLACK)
            text(42, 91, "DRUG WARS", WHITE)
            text(234, 110, "2.00", WHITE)
            text(34, 135, "TI-Nspire CX II Edition", CYAN)
            text(33, 158, "Based on J.M. TI version", CYAN)

            text(66, 184, "ENTER: START", YELLOW)
            text(190, 184, "ESC: QUIT", GRAY)
            end_screen()
    ...
    def coat_screen(self):
        ...
        # Inventory list on the left, coat artwork on the right.
        y = 47
        for i in range(6):
            text(9, y, DRUGS[i], WHITE)
            text(130, y, str(self.inv[i]), CYAN)
            y += 20

        show_asset("trenchcoat", 210, 42)
    ...
    def travel(self):
        while True:
            ...
            # Subway transition with embedded background art.
            begin_screen()
            header("SUBWAY")
            show_asset("subwaybg", 168, 42)
            text(14, 82, "Traveling to", WHITE)
            text(14, 112, self.location_name(), CYAN)
            footer("ENTER: Arrive")
            end_screen()
            wait_enter_or_esc()
    ...
    def random_event(self, event):
        ...
        if event == 1:
            ...
            message_with_asset("MARKET EVENT",
                    ["CHEAP HEROIN!", "Police seizure", "hits the market."],
                    "police_car", YELLOW, 190, 55)
            ...
            if self.free_space() >= 8:
                ...
                message_with_asset("SUBWAY FIND",
                        ["YOU FOUND DRUGS!", str(amount) + " " + DRUGS[which],
                         "on a dead guy."],
                        "dead_body", YELLOW, 195, 58)
    ...
    def gun_offer(self):
        ...
        choices = [
            ("BERETTA", "gun"),
            ("SAT. NIGHT SPECIAL", "gun_snub"),
            (".44 MAGNUM", "gun_44")
        ]
        gun, asset = choices[randint(0, 2)]

        selected = 0
        while True:
            begin_screen()
            header("GUN OFFER")
            show_asset(asset, 192, 52)
            text(12, 58, gun, YELLOW)
            text(12, 84, "$400", GREEN)
            text(12, 112, "Buy it?", WHITE)
            opts = ["YES", "NO"]
            for i in range(2):
                y = 145 + i * 24
                if i == selected:
                    text(18, y, "> " + opts[i], YELLOW)
                else:
                    text(18, y, "  " + opts[i], WHITE)
            footer("UP/DOWN", "ENTER: Select")
            end_screen()
            ...
            if k == "up" or k == "down":
                ...
                self.guns += 1
                self.cash -= 400
                message_with_asset("GUN PURCHASED",
                        ["Guns: " + str(self.guns),
                         "Free space: " + str(self.free_space())],
                        asset, GREEN, 195, 62)
                return
    ...
    def police_chase(self, deputies):
        ...
        message_with_asset("POLICE!",
                ["OFFICER HARDASS", "+ " + str(deputies) + " deputies",
                 "are after you!"],
                "police_car", RED, 190, 55)
    ...
    def police_fire(self):
        if randint(0, 1) == 0:
            message_with_asset("POLICE FIRE",
                    ["They fired...", "and MISSED!"],
                    "police_car", GREEN, 190, 55)
            return False
    ...
    def police_victory(self):
        ...
        message_with_asset("CHASE OVER",
                ["You got them all!", "Found $" + str(loot),
                 "on Officer Hardass."],
                "police_car", GREEN, 190, 55)

        if self.cash >= 1200 and self.damage > 0:
            message_with_asset("DOCTOR",
                    ["Treatment available", "$1000 to heal."],
                    "doctor", CYAN, 210, 47)
